...
{
  //----------------------------------------------------------------------
  void PredictionInterp::ShiftedPreds::FillRemaps()
  {
    // Copy fits into the remapped indexing order. TODO this is very ugly. Best
    // would be to generate things in this order natively.

    for(bool nubar: {false, true}){
      const auto& from = nubar ? fitsNubar : fits;
      auto& to = nubar ? fitsNubarRemap : fitsRemap;

      // Allocate the space
      to.resize(from.size());
      for(auto& it: to){
        it.resize(from[0][0].size());
        for(auto& it2: it) it2.resize(from[0].size(), Coeffs(0, 0, 0, 0));
      }

      // And copy with the transposed indices
      for(unsigned int i = 0; i < to.size(); ++i)
        for(unsigned int j = 0; j < to[i].size(); ++j)
          for(unsigned int k = 0; k < to[i][j].size(); ++k)
            to[i][j][k] = from[i][k][j];
  ...
  //----------------------------------------------------------------------
  std::vector<std::vector<PredictionInterp::Coeffs>>
  PredictionInterp::FitRatios(const std::vector<double>& shifts,
  ...
  //----------------------------------------------------------------------
  std::vector<std::vector<PredictionInterp::Coeffs>>
  PredictionInterp::FitComponent(const std::vector<double>& shifts,
  ...
  //----------------------------------------------------------------------
  std::vector<std::vector<PredictionInterp::Coeffs>>
  PredictionInterp::FitSumComponent(const std::vector<ShiftedPreds>& sps,
                                        Flavors::Flavors_t flav,
                                        Current::Current_t curr,
                                        Sign::Sign_t sign) const
  { 
  ...
  //----------------------------------------------------------------------
  void PredictionInterp::InitFitsSumHelper(std::vector<ShiftedPreds>& sp,
                                           std::vector<std::vector<std::vector<Coeffs>>>& fits,
                                           Sign::Sign_t sign) const
  {
    fits.resize(kNCoeffTypes);
  ...
  //----------------------------------------------------------------------
  Spectrum PredictionInterp::ShiftSpectrum(const Spectrum& s,
                                           CoeffsType type,
                                           bool nubar,
                                           const SystShifts& shift) const
  {
  ...
  //----------------------------------------------------------------------
  void PredictionInterp::Derivative(osc::IOscCalculator* calc,
                                    const SystShifts& shift,
                                    double pot,
                                    std::unordered_map<const ISyst*, std::vector<double>>& dp) const
  {
  ...
  std::unique_ptr<PredictionInterp> PredictionInterp::LoadFrom(TDirectory* dir)
  {
    ...
    assert(tag);
    assert(tag->GetString() == "PredictionInterp"); // backwards compatibility
                                      ...
                                      std::vector<const ISyst*> veto)
  {
    ...
    if(hSystNames){
      for(int systIdx = 0; systIdx < hSystNames->GetNbinsX(); ++systIdx){
        ...
        auto syst = SystRegistry::ShortNameToSyst(sp.systName);
